...
export class P2PNet {
  ...
  screenCalls: Map<string, MediaConnection> = new Map();
  _events: { [key: string]: Function[] } = {};

  _heartbeatTimer: any = null;
  _watchdogTimer: any = null;
  ...
  emit(event: string, ...args: any[]) {
    if (this._events[event]) {
      this._events[event].forEach(h => {
        try { h(...args); } catch (e: any) { this._audit('ERR', `Event '${event}' error: ${e.message}`); }
      });
    }
  }

  _audit(category: any, message: string, extra: any = null) {
  ...
  _connectToPeer(remotePeerId: string, callbacks: any = {}) {
  ...
  _handleIncomingConnection(conn: DataConnection) {
  ...
  _bindDataEvents(conn: DataConnection, peerRecord: any) {
  ...
  _handleSystemPacket(senderPeerId: string, packet: any) {
  ...
  send(data: any, targetPeerId: string | null = null) {
  ...
  broadcast(data: any, excludePeerIds: string[] = []) {
  ...
  call(remotePeerId: string, stream: MediaStream | null, metadata: any = {}) {
  ...
  callScreen(remotePeerId: string, stream: MediaStream | null, senderName = '') {
  ...
  startScreenShare(stream: MediaStream, myName = '') {
